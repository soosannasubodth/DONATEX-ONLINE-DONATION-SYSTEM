...
export default function NgOAccountProfile() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.organization_name !== profile.organization_name) data.organization_name = form.organization_name;
    if (form.registration_number !== profile.registration_number) data.registration_number = form.registration_number;
    ...
    <div style={styles.container}>
      ...
      {!loading && (
        <div style={styles.profileBox}>
          {!editing ? (
            <div>
              <p><b>Contact Name:</b> {profile.full_name}</p>
              <p><b>Email:</b> {profile.email}</p>
              <p><b>Organization:</b> {profile.organization_name}</p>
              <p><b>Registration:</b> {profile.registration_number}</p>
              <p><b>Address:</b> {profile.address || "-"}</p>
              <p><b>Phone:</b> {profile.phone_number || "-"}</p>
              <button onClick={() => setEditing(true)} style={styles.editBtn}>Edit Account</button>
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
              <input type="text" name="full_name" value={form.full_name} onChange={handleChange} placeholder="Contact Name" style={styles.input} />
              ...
              <input type="text" name="organization_name" value={form.organization_name} onChange={handleChange} placeholder="Organization Name" style={styles.input} />
              <input type="text" name="registration_number" value={form.registration_number} onChange={handleChange} placeholder="Registration Number" style={styles.input} />
...
const styles = {
  ...
  profileBox: { background: "#fff", padding: "20px", borderRadius: "8px", maxWidth: "600px" },
